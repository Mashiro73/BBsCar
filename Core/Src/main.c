  ...
  if (htim->Instance == TIM3)
  {
    ...
    else if(echostate == 1)
    {
      ...
      else
      {
        ...
        flagCarStop = 0; // 如果回声时间大于1000ms，认为车辆继续行驶
        echostate = 0;   // 重置状态
      }
    }
  }
...
void TrigOut(void)
{
  HAL_GPIO_WritePin(Trig_GPIO_Port, Trig_Pin, GPIO_PIN_SET); // Set the trigger pin high
  HAL_Delay(1);
  HAL_GPIO_WritePin(Trig_GPIO_Port, Trig_Pin, GPIO_PIN_RESET); // Set the trigger pin low
  HAL_Delay(65);
}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  while (1)
  {
    ...
    sprintf(LCDBuf, "cnt:%d     ", cntEcho);
    LCD_ShowString(0, 13 * 5, LCDBuf, WHITE, BLACK, 12, 0);
    sprintf(LCDBuf, "ecosta:%d     ", echostate);
    LCD_ShowString(0, 13 * 6, LCDBuf, WHITE, BLACK, 12, 0);
    sprintf(LCDBuf, "flagstop:%d     ", flagCarStop);
    LCD_ShowString(0, 13 * 7, LCDBuf, WHITE, BLACK, 12, 0);
    /* USER CODE END WHILE */
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, LCD_CS_Pin | LCD_MOSI_Pin | STBY_Pin | AIN1_Pin | AIN2_Pin | Trig_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : LCD_CS_Pin LCD_MOSI_Pin STBY_Pin AIN1_Pin
                           AIN2_Pin Trig_Pin */
  GPIO_InitStruct.Pin = LCD_CS_Pin | LCD_MOSI_Pin | STBY_Pin | AIN1_Pin | AIN2_Pin | Trig_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
